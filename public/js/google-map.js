$(document).ready(function() {
    ...
    if ($('#map-object').length !== 0) {
        var mapCenter = new google.maps.LatLng(-16.5231338,-68.1157638);
        ...
        $.ajax({
            ...
            'success': function (data) {
                ...
                $.each(data, function (index, value) {
                    ...
                    var thumbnail = value.thumbnail.replace("/tmp","");
                    var image = value.imagen.replace("/tmp","");
                    var markerTemplate = 
                        '<div id="marker-' + value.id + '" class="marker">' +
                            '<div class="marker-inner">' + 
                                '<span class="marker-image" style="background-image: url(' + thumbnail + ');"></span>' +
                            '</div>' +
                            verified + 
                            price + 
                        '</div>';
                    ...
                    google.maps.event.addListener(marker, "click", function () {
                        var c = '<div class="infobox"><div class="infobox-close"><i class="fa fa-close"></i></div>' +
                            '<h3 class="infobox-title"><a id="yourLink"  href="">' + marker.data.title + '</a></h3>' +
                            '<h4 class="infobox-address"></h4>' +
                            '<div class="infobox-content">' +
                            '<div class="infobox-image" style="background-image: url(' + image + ');"><ul><li><a href="#"><i class="fa fa-facebook"></i></a></li><li><a href="#"><i class="fa fa-twitter"></i></a></li><li><a href="#"><i class="fa fa-google"></i></a></li></ul></div>' +
                            '<div class="infobox-body"><div class="infobox-body-inner"><div class="infobox-category tag">Hamburguesas</div><p><strong>' + marker.data.description + '</p></div>' +
                            '<div  class="infobox-more"><a  href="https://migas.app/branches/">Read More <i class="fa fa-chevron-right"></i></a></div>' +
                            '</div>' +
                            '<div>';
                        $('#yourLink').attr("href", "https://migas.app/branches/"+ value.id);
